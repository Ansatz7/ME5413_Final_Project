...
PATROL_WAYPOINTS = [
    ...
    # 朝向 = 行进方向（(0,0)→(3.5,0)，向东）
    ( 3.5,  0.0,   0, '#1  入口'),

    # ── #2~#5  房间 A ────────────────────────────────────────────────
    ( 3.5,  7.5,  90, '#2  A-左墙中'),    # (3.5,0)→(3.5,7.5)，向北
    ( 3.5, 16.0,  90, '#3  A-左墙上'),    # (3.5,7.5)→(3.5,16)，向北
    (11.5, 16.0,   0, '#4  A-通道顶左'),  # (3.5,16)→(11.5,16)，向东
    (11.5, 13.0, -90, '#5  A-通道上左'),  # (11.5,16)→(11.5,13)，向南

    # ── #6~#9  穿越上连通口 → 房间 B ────────────────────────────────
    (13.5, 13.0,   0, '#6  B-通道上右'),  # (11.5,13)→(13.5,13)，向东
    (13.5, 16.0,  90, '#7  B-通道顶右'),  # (13.5,13)→(13.5,16)，向北
    (21.3, 16.5,   0, '#8  B-右墙上'),    # 离开角落，避免贴墙卡住
    (21.3,  7.5, -90, '#9  B-右墙中'),    # (22,16)→(22,7.5)，向南

    # ── #10~#13  房间 B 下半 → 穿越下连通口 ─────────────────────────
    (21.3, -1.0, -90, '#10 B-右墙下'),    # (22,7.5)→(22,-1)，向南
    (13.5, -1.0, 180, '#11 B-通道底右'),  # (22,-1)→(13.5,-1)，向西
    (13.5,  2.0,  90, '#12 B-通道下右'),  # (13.5,-1)→(13.5,2)，向北
    (11.5,  2.0, 180, '#13 A-通道下左'),  # (13.5,2)→(11.5,2)，向西

    # ── #14  交接点 ──────────────────────────────────────────────────
    # (11.5,2)→(7.5,-1.9)，atan2(-3.9,-4)≈-136°
    ( 7.5, -1.9,-136, '#14 leave_level_1'),
]

# ── 各巡逻点 xy_goal_tolerance ────────────────────────────────────────
# 门洞/连通口附近需要对准，其余放宽避免被箱子卡住
WAYPOINT_TOLERANCES = [
    0.35,  # #1  入口（门洞窄）
    0.8,   # #2  A-左墙中
    0.8,   # #3  A-左墙上
...
class Level1Patrol:
    ...
    def _navigate_to(self, x, y, yaw_deg, label, tolerance=0.8):
        rospy.loginfo('[level1_patrol] → %s  (%.1f, %.1f, %d°)  tol=%.2fm',
                      label, x, y, yaw_deg, tolerance)
    ...
    def run(self):
        ...
        for i, (x, y, yaw_deg, label) in enumerate(PATROL_WAYPOINTS):
            if rospy.is_shutdown():
                break
            tol = WAYPOINT_TOLERANCES[i] if i < len(WAYPOINT_TOLERANCES) else 0.8
            rospy.loginfo('[level1_patrol] [%d/%d] 前往 %s', i + 1, total, label)
            ok = self._navigate_to(x, y, yaw_deg, label, tolerance=tol)
            if ok:
                success += 1
