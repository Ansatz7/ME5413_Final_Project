...
PATROL_WAYPOINTS = [
    # ── 穿过入口墙（map x=2.5，门洞 y∈[-0.82,0.68]）进入大厅 ───────
    # 门洞宽 1.5m，先直行穿越
    ( 3.5,  0.0,   0, '入口-穿越'),
    ...
    # ── 交接点（队友接管）────────────────────────────────────────────
    ( 7.5, -1.9, -90, 'leave_level_1'),   # 朝向 (0,-1)，yaw=-π/2
]

# ── 参数 ─────────────────────────────────────────────────────────────
GOAL_TIMEOUT   = 60.0   # 单点导航超时（秒）
ARRIVE_PAUSE   = 1.0    # 到达后停留时间（秒，让 box_counter 多采几帧）
...
class Level1Patrol:
    ...
    def _navigate_to(self, x, y, yaw_deg, label):
        rospy.loginfo('[level1_patrol] → %s  (%.1f, %.1f, %d°)', label, x, y, yaw_deg)
